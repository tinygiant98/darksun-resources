...
void main()
{
    ...
    effect eLink=EffectSilence();
    eLink=EffectLinkEffects(eLink, EffectVisualEffect(VFX_DUR_CESSATE_NEGATIVE));
    eLink=EffectLinkEffects(eLink, EffectSpellFailure());
    eLink=ExtraordinaryEffect(eLink);
    int nLevel=GetEffectivePsionicLevel(oPC, FEAT_PSIONIC_MIND_THRUST);
    ...
    if (!PowerCheck(oPC, nCost, nPowerScore, FEAT_PSIONIC_MIND_THRUST)) return;

    int nDuration=GetEnhancedDuration(1+nLevel/2);

    SignalEvent(oTarget, EventSpellCastAt(oPC, SPELL_PSIONIC_MIND_THRUST, TRUE));
